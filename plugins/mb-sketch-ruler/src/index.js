...
export default class SketchRuler extends PureComponent {
  ...
  render() {
    ...
    return (
      <div id="mb-ruler" className={`cmb-ruler ${isShowReferLine ? "show-line" : ""}`}
           onContextMenu={this.preventDefault}>
        {/* 水平方向 */}
        <RulerWrapper width={width} height={thick} start={startX} lines={horLineArr} selectStart={x}
                      selectLength={w} {...commonProps} />
        {/* 竖直方向 */}
        <RulerWrapper width={thick} height={height} start={startY} lines={verLineArr} selectStart={y} selectLength={h}
                      vertical {...commonProps} />
        <a className={`corner${cornerActive ? ' active' : ''}`} style={{backgroundColor: bgColor}}
